...
exports.getGoalTransactions = async (req, res) => {
  try {
    const transactions = await fetchGoalTransactions(req.userId, req.params.id, req.query);
...
exports.createGoalTransaction = async (req, res) => {
  try {
    const transaction = await createTransactionForGoal(req.userId, req.params.id, req.body);
