...
export default function AddRecurringTransactionPage() {
  ...
  }, [amount, currency, selectedCategory, note, startDate, endDate, frequency]);
  ...
  const validate = () => {
    ...
    if (!startDate || dayjs(startDate).isBefore(today)) {
      newErrors.startDate = 'Дата початку не може бути раніше поточної';
    }
    if (endDate) {
      if (dayjs(endDate).isBefore(startDate)) {
        newErrors.endDate = 'Дата завершення має бути не раніше дати початку';
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    if (endDate) data.endDate = dayjs(endDate).format('YYYY-MM-DD');

    try {
      await axios.post('http://localhost:5000/api/recurring-transactions', data, {
        headers: {
  ...
  return (
    <div className="max-w-xl mx-auto mt-10 bg-white shadow-lg rounded-2xl p-10">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Нова регулярна транзакція</h1>
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm text-gray-700 mb-1">Дата першого виконання</label>
          <DatePicker
            value={startDate}
      ...
      >
        ...
        <DialogActions sx={{ justifyContent: 'flex-end' }}>
          ...
          <Button
            variant="contained"
            color="error"
            onClick={() => navigate(from === 'budget' ? '/budgets' : '/recurring-transactions')}
          >
            Вийти
          </Button>
        </DialogActions>
